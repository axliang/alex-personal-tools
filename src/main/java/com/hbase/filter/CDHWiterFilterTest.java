...
public class CDHWiterFilterTest {
...
//	String tableName = "veh_passrec_sjscsj";
	...
	public static void main(String[] args) {
	 ...
	 new CDHWiterFilterTest().zdcTest();
	...
	private void zdcTest(){
		...
		try {
			List<Filter> fiterTotal= new ArrayList<Filter>();
		
			SingleColumnValueFilter filterHphm = new SingleColumnValueFilter(
					Bytes.toBytes(FAMALIY),
					Bytes.toBytes("hphm"),
					CompareFilter.CompareOp.NOT_EQUAL,
					new BinaryComparator(Bytes.toBytes("鲁D12345")));
			
			//要使用作为条件的列，如果这一列本身就不存在，那么如果为true，这样的行将会被过滤掉，如果为false，这样的行会包含在结果集中
			filterHphm.setFilterIfMissing(true);
			SingleColumnValueFilter filterHpzl = new SingleColumnValueFilter(
					Bytes.toBytes(FAMALIY),
					Bytes.toBytes("hpzl"),
					CompareFilter.CompareOp.NOT_EQUAL,
					new BinaryComparator(Bytes.toBytes("02")));
			filterHpzl.setFilterIfMissing(true);
			List<Filter> filter_white = new ArrayList<Filter>();
			filter_white.add(filterHphm);
			filter_white.add(filterHpzl);
			
			
			fiterTotal.add(new FilterList(Operator.MUST_PASS_ONE,filter_white));
			
			
			SingleColumnValueFilter hpzlFilter = new SingleColumnValueFilter(
					Bytes.toBytes(FAMALIY),
					Bytes.toBytes("hpzl"),
			...
			fiterTotal.add(hpzlFilter);

			
			
//			FilterList  firstCarFilterList=new FilterList(Operator.MUST_PASS_ALL);
//			firstCarFilterList.addFilter(filterHphm);
//			firstCarFilterList.addFilter(filterHpzl);
//			
//	
//			SingleColumnValueFilter secondCarFilterHphm = new SingleColumnValueFilter(
//					Bytes.toBytes(FAMALIY),
//					Bytes.toBytes("hphm"),
//					CompareFilter.CompareOp.NOT_EQUAL,
//					new BinaryComparator(Bytes.toBytes("鲁D23456")));
//			
//			//要使用作为条件的列，如果这一列本身就不存在，那么如果为true，这样的行将会被过滤掉，如果为false，这样的行会包含在结果集中
//			filterHphm.setFilterIfMissing(true);
//			SingleColumnValueFilter secondFilterHpzl = new SingleColumnValueFilter(
//					Bytes.toBytes(FAMALIY),
//					Bytes.toBytes("hpzl"),
//					CompareFilter.CompareOp.NOT_EQUAL,
//					new BinaryComparator(Bytes.toBytes("02")));
//			
//			FilterList secondCarFilterList=new FilterList(Operator.MUST_PASS_ALL);
//			secondCarFilterList.addFilter(secondCarFilterHphm);
//			secondCarFilterList.addFilter(secondFilterHpzl);
//			
//	
//			
//			SingleColumnValueFilter hpzlFilter = new SingleColumnValueFilter(
...
//					new BinaryComparator(Bytes.toBytes("02")));
//			
//			FilterList  fiterWhiteCars=new FilterList(Operator.MUST_PASS_ALL); 
//			fiterWhiteCars.addFilter(firstCarFilterList);
//			fiterWhiteCars.addFilter(secondCarFilterList);
			
			
			HTable table = new HTable(config, tableName);
			Scan scan = new Scan();
			scan.setFilter(new FilterList(Operator.MUST_PASS_ALL,fiterTotal));
//			scan.addColumn(Bytes.toBytes("v"), Bytes.toBytes("zdc"));
			...
			scan.addColumn(Bytes.toBytes("v"), Bytes.toBytes("hphm"));
