...
public class GLBS_CDT
{
	 ...
	 * 
	 * @param datain
	 * @param confidence  置信区间 取值为  0.90,0.95,0.975,0.99
	 * @return
	 */
	public  List<Double>  filter( List<Double> datain,double  confidence ){
		MathsUtils mathsutils = new MathsUtils();
		boolean flag_GoOn_Clipe = true;  // 是否继续删除异常数据的标志
		Collections.sort(datain);  // 先排序
		while (flag_GoOn_Clipe) {
			if (datain.size() < 3){
				flag_GoOn_Clipe = false;  // 小于三个  不满足格拉布斯删除要求
				break;
			}
			double average = mathsutils.calcAverage(datain);
			double standard = mathsutils.calcStandard(datain, datain.size(),average);
			/** 确定残差与标准差的比值 **/
			double dubMin = (average - datain.get(0)) / standard;
			double dubMax = (datain.get(datain.size() - 1) - average)/ standard;

			//获得临界值
			double threshold=0d;
				if(datain.size() >100)
					threshold= this.map.get(101 + confidence);
				else{
					threshold= this.map.get(datain.size() + confidence);
				}
			if (dubMin >= dubMax) {
				/** 步骤1 确定可疑值 **/
				if (dubMin > threshold) {
					System.out.println(" flit value: "+datain.get(0));
					datain.remove(0);
					
				} else {
					flag_GoOn_Clipe = false;
				}

			} else {
				if (dubMax > threshold) {
					System.out.println(" flit value: "+datain.get(datain.size()-1));
